...
if(!file.exists("mimiciv_shiny/icu_cohort.rds")){
  icustays_tble %>% 
  #(i) keep only the first ICU stay of each unique patient
  group_by(subject_id) %>%
  slice_min(intime) %>%
  #(ii) merge in admissions and patients data
  ...
  left_join(admissions_tble %>%
              ...
              mutate(died = 1,
                     deathtime_upd = last(dischtime)) %>%
              slice_max(dischtime) %>%
              select(subject_id, died, deathtime_upd),
            by  = "subject_id") %>%
  ...
  #(iv) merge in labevents and chartevents tables
  ...
  mutate(died = ifelse(is.na(died), 0, died),
         ...
         admit_to_death = difftime(date(deathtime_upd), date(admittime),
                                   units = "days"),
