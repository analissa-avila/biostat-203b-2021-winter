...
**Note:** I reordered the steps so that filtering to adults (step iii) is done after the query results are collected into memory as a tibble (step vi). When I tried to filter by age before collecting the results into memory, I would get an error that the 'server closed the connection unexpectedly'. 
```{r}
...
icustays_tble %>% 
  ...
  #merge in admissions and patients data
  left_join(admissions_tble %>%
              select(!(deathtime)),
            by = c("subject_id", "hadm_id")) %>%
  #get time of death data
  left_join(admissions_tble %>%
...
### Solution:
Run the app to explore univariate distributions of demographic variables, lab measurements, and vitals in the ICU cohort. Both graphical and numerical summaries are provided.
```{r, warning=FALSE, eval=FALSE}
runApp("mimiciv_shiny")
```
