...
The `admissions` data dictionary states that discharge time and death time should be the same, but may differ due to typographical errors. We first look at these to variables to see which we should use to determine death time.  
...
In addition to the 13 missing death times, there are 634 discrepant discharge and death times.  In all 634 cases, *only the time is discrepant*. The dates are exactly the same. *Since the dates are the same in both variables and `dischtime` does not have any missing values, we will use `dischtime` to analyze death dates*.
```{r}
...
The graph shows that the vast majority of people did not day during any hospitalization.
...
The anchor age of 0 may be used to denote missing data, therefore I filtered out anchor ages of zero and created a new graph. These anchor ages span from 18 to 91. 
```{r}
patients_tab %>%
...
Per question 8, we only need the first lab measurements during an ICU stay. Therefore, we subset to first lab measurements during each stay.
```{r}
...
#                 fread(str_c(mimic_p, "/icu/chartevents.csv.gz"),
#                       select = c("subject_id", "hadm_id", "itemid", 
#                                  "charttime", "valuenum"), 
#                       nThread = 4))
#   chartevents_tab %>%
...
# }
# 
# system.time(chartevents_tab <- fread("chartevents_icustays.csv.gz"))
# chartevents_tab %>%
#   as.tibble() %>%
#   print(width = Inf)
...
Read in the chart item dictionary
```{r}
ditems_tab <- read_csv(str_c(mimic_p, "/icu/d_items.csv.gz")) %>%
  print(width = Inf)
```

Get relevant `itemid`s for vitals
```{r}
chart_list <- c("Heart Rate", "Temperature Fahrenheit", "SpO2 Desat Limit",
                "Respiratory Rate")

ditems_tab_sub <- ditems_tab %>%
  filter(label %in% chart_list)

## Non Invasive Blood Pressure systolic, Manual Blood Pressure Systolic (Right/Left), Arterial Blood Pressure systolic, ART BP systolic - WHICH ONE?

## Non Invasive Blood Pressure mean, Mean BP (VAD), Arterial Blood Pressure mean, ART BP mean, IABP Mean, HM II- Mean BP - WHICH ONE?

#RR (set), RR(sponaneous), RR(total) -- WHICH ONE?
...
#### Solution:
- vitals ?
- `admittime` to death OR icu `intime` to death?
```{r}
...
icustays_tab_first <- icustays_tab %>% 
  ...
  mutate(died = ifelse(is.na(died), 0, died),
         ...
         days_admit_death = difftime(date(death_time), date(admittime), 
                                  units = "days"),
         #indicator: died within 30 days of admission
         died_in_30days = ifelse(days_admit_death > 30 | 
                                   is.na(days_admit_death), 0, 1)) %>%
  print(width = Inf)
