...
Per question 8, we only need the first lab measurements during an ICU stay.  
Therefore, we subset to first lab measurements during each stay.  
Transform data to wide, so that each measurement has its own column.
```{r}
if(!file.exists("labevents_icustays.csv.gz")){
...
If we were able to load the original `chartevents` file, we would use the following code to subset it to only the relevant `itemid`s. (Relevant labels are per Dr. Zhou's lecture)  
...
Instead, we just need to read in the pre-filtered `chartevents` data set.  
Per question 8, subset to only include first vitals measurements during icu stay for each patient
```{r}
if(!file.exists("chartevents_filtered.csv.gz")){
