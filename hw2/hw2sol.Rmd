...
There are 7 patients with more than 1 death record (i.e. more than 1 hospitalization in which the patient was flagged as having died). There are 12 patients who have a reported death, but a missing death time.  
The graph shows that the vast majority of people did not day during any hospitaliztion.
...
#number of death records with a missing deathtime  
admissions_tab %>%
  group_by(subject_id) %>%
  mutate(death = sum(hospital_expire_flag)) %>%
...
```{r}
dlabitems_tab <- read_csv(str_c(mimic_p, "/hosp/d_labitems.csv.gz")) %>%
  print(width = Inf)
```
